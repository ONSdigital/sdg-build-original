    ...
    for group in edges['To'].unique():
        ...
        while len(parents0) > 0:
            ...
            for p in parents0:
                ...
                parents0 = parents
                # note that this does not interrupt the 'for p in parents0' loop 
                # - the loop will go through p in the 'original' parents0 before
                # ending
                #
                # surely this would only step up to find great-grand parents for
                # the final p in the original parents0?
                #
                # consider p1 and p2 in the original parents0
                # p1 grandparents added to list
                # parents0 set as grandparents of p1
                # [next p in original parents0]
                # p2 grandparents added to list
                # parents0 set as grandparents of p2
                # so grandparents of p1 not checked?
                #
                # ![as this is inside a while loop with condition parents0 > 0,
                # this means that the for loop goes through the parents first,
                # then all the identified grandparents (the new parents0) and
                # so on for great-grand parents, etc., each time finding parents
                # of elements of that list and adding them to grand_parents]
